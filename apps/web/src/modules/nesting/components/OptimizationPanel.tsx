...
export function OptimizationPanel({ bestRotation, bestArrangement, estimatedSavings, optimizationScore, warnings }: OptimizationPanelProps) {
  const rows = [
    ["Best Rotation", bestRotation],
    ["Best Arrangement", bestArrangement],
  ...
  return (
    ...
    >
      <h2 style={{ margin: 0, color: "#f8fafc", fontSize: 18, fontWeight: 700 }}>Optimization Report</h2>
      <div style={{ display: "grid", gap: 10, marginTop: 14 }}>
        {rows.map(([label, value]) => (
          <div key={label} style={{ display: "flex", justifyContent: "space-between", gap: 8, padding: "10px 12px", borderRadius: 14, background: "rgba(15, 23, 42, 0.72)" }}>
            ...
            <span style={{ color: label === "Warnings" ? "#fbbf24" : "#f8fafc", fontWeight: 700, textAlign: "right" }}>{value}</span>
          </div>
        ))}
      </div>
    </section>
